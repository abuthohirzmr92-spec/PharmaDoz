...
export function LowStockCard({ branchId }: { branchId?: string }) {
  ...
  // Approximate branch filter: only show products that appear in branch transactions
  ...
  let products = getLowStock();
  if (branchProductIds) {
    products = products.filter((p) => branchProductIds.has(p.id));
  }
