...
import { Pill, Plus, Search, RotateCcw } from "lucide-react";
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      if (productRepo.isConnected) {
        ...
        // We need totalStock from batches. Use getProducts for aggregated data,
        // but let's fetch batches separately for a more complete view.
        ...
        const mapped: ProductRow[] = rawProducts.map((p) => ({
          ...
          minStock: p.minStock,
          totalStock: 0, // computed from batches — read-only
          isActive: p.isActive,
        }));
