...
// Vercel Cron entrypoint — subscription lifecycle sweep.
// Not cached; must run per-request. Bounded by maxDuration.
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    const result = await createPrivilegedScheduler().runSubscriptionSweep(runDate, nowISO);
    // eslint-disable-next-line no-console -- structured observability log (info)
    console.log(
      JSON.stringify({
        ts: nowISO, level: "info", service: "scheduler", op: "subscription_sweep",
        correlationId: `scheduler:subscription_sweep:${runDate}`,
    ...
    console.error(
      JSON.stringify({
        ts: nowISO, level: "error", service: "scheduler", op: "subscription_sweep",
        correlationId: `scheduler:subscription_sweep:${runDate}`,
